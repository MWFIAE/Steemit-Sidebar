...
<p id="mw-username-p"><input id="mw-username" type="text" value="{{settings.username}}" placeholder="Username"/></p>
...
SteemPower: <a href="https://steemit.com/@{{user.name}}/transfers">{{user.sp}}</a>
</p>
<ul class="mw-ul">
{{temp.links}}
</ul>
...
<div id="tabs">
  ...
  <div id="tabs-1">
    <table>
      <tr><td>Bar Color High</td><td><input id="mw-barColorHigh" class="mw-inline" type="color" value="{{settings.barColorHigh}}"/></td></tr>
  ...
  <div id="tabs-2" style="overflow-y: auto">
    <input class="mw-button" type="button" id="mw-restore-links" value="Restore Links"/>
...
unsafeWindow.MWSidebar ={
    ...
    updateDisplay :function(target, user) {
        ...
        let links = MWSidebar.settings.links.reduce((result,link)=>{
            return (result=={}?"":result) + MWSidebar.helper.fillTemplate(TEMPLATE_LINK, {link: link, user: user});
        },"");
        temp = {
            target: target,
            links: links,
        ...
        content = MWSidebar.helper.fillTemplate(TEMPLATE_WITH_USER, {settings: MWSidebar.settings, user: user, temp:temp});
